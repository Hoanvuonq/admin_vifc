...
export const LanguageSwitcher = ({
  ...
  return (
    <div className={cn("relative text-[11px] my-1 font-bold", className)} ref={dropdownRef}>
      <button
      ...
      >
        <div className="flex items-center gap-1.5">
          ...
          <div className="flex flex-col items-start leading-none gap-0.5">
            <span className="text-gray-600 group-hover:text-gray-900 tracking-wider">{currentLang.display}</span>
          </div>
        </div>
        <ChevronDown size={12} className={cn("text-gray-600 transition-transform duration-300", isOpen ? "rotate-180 text-orange-500" : "rotate-0")} />
      </button>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="px-4 py-2 text-[9px] font-bold text-gray-600 uppercase mb-1 border-b border-slate-50">Quốc gia / Ngôn ngữ</div>
              ...
              {locales.map((lang) => (
                <button
                  ...
                  className={cn(
                    ...
                    currentLocaleCode === lang.code ? "bg-orange-50 text-orange-600" : "text-gray-600 hover:bg-slate-50 hover:text-gray-900",
                  )}
                >
                    ...
                    <span
                      className={cn(
                        ...
                        currentLocaleCode === lang.code
                          ...
                          : "bg-slate-100  text-gray-700 group-hover:bg-orange-100 group-hover:text-orange-600",
                      )}
                    >
                      {lang.display}
                    </span>
                  ...
                  {currentLocaleCode === lang.code && <motion.div layoutId="active-lang" className="absolute left-0 w-1 h-2/3 bg-orange-500 rounded-r-full" />}
                </button>
              ))}
            </div>
          </motion.div>
