    ...
    >
      ...
      {metrics.length > 0 && !hideMetricsInPage && (
        <div className="flex-1 flex justify-end">
          <div className="flex items-center gap-2 p-2 bg-gray-100/30 backdrop-blur-2xl rounded-2xl border border-white/80 shadow-[inset_0_2px_15px_rgba(0,0,0,0.02),0_20px_40px_-20px_rgba(0,0,0,0.05)] relative z-10 transition-all duration-500 hover:bg-gray-100/50">
            {metrics.map((metric, index) => (
              <Fragment key={index}>
                <MetricItem label={metric.label} value={metric.value} icon={metric.icon} color={metric.color} isMoney={metric.isMoney} suffix={metric.suffix} />
                {index < metrics.length - 1 && <div className="w-px h-12 bg-linear-to-b from-transparent via-gray-200/50 to-transparent mx-2" />}
              </Fragment>
            ))}
          </div>
        </div>
