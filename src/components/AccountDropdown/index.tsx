...
export const ADMIN_MENU_ITEMS: MenuItem[] = [{ key: "admin_dash", label: "Admin Dashboard", href: "/admin/dashboard", icon: <ShieldCheck size={16} /> }];
  ...
  const Trigger = (
    ...
    >
      <div className="relative">
        ...
        >
          {mounted && isActuallyAuthenticated && userData.image ? (
            <Image src={userData.image} alt="avatar" width={32} height={32} className="w-full h-full object-cover" />
          ) : (
            <div className="w-full h-full flex items-center justify-center bg-gray-100">
              <Image src="/icons/icon_sidebar2.png" alt="user icon" width={14} height={14} className="object-contain opacity-80" />
            </div>
          )}
        </div>
      ...
      <div className="hidden md:flex flex-col items-start min-w-0 pr-1">
        <span className={cn("font-bold text-[11px] uppercase tracking-tighter px-1 truncate max-w-30 leading-tight transition-colors italic text-gray-900")}>
        ...
        {mounted && canShowLoyalty && (
          <div className="flex items-center gap-1 opacity-90 group-hover:opacity-100 transition-all pl-1">
            ...
            <span className="text-orange-400 font-bold text-[9px] tabular-nums tracking-tighter">{formatNumber(loyaltyData?.totalPointsAllShops || 0)}</span>
          </div>
        )}
      </div>
      ...
      <ChevronDown
        size={12}
        className={cn("transition-all hidden md:flex duration-500 ml-0.5 opacity-40 group-hover:opacity-100 group-hover:rotate-180 text-gray-500")}
      />
    </div>
  );
  ...
  return (
    <AppPopover trigger={Trigger} className="w-76 mt-2 animate-in fade-in zoom-in-95 duration-300" align="right" contentClassName="overflow-visible ">
            ...
            <div className="flex items-start  justify-between gap-4">
              <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="w-14 h-14 all-center rounded-2xl overflow-hidden shadow-xl border-2 border-orange-500 bg-white ring-4 ring-orange-500/10">
                    {userData.image ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center bg-gray-50 text-gray-500">
                        <Image src="/icons/icon_sidebar2.png" alt="user icon" width={10} height={10} className="object-contain opacity-80" />
                      </div>
                    )}
                  </div>
                ...
                <div className="flex flex-col min-w-0">
                  <h4 className="font-extrabold text-gray-900 text-base truncate leading-tight uppercase tracking-tighter italic">{userData.name}</h4>
                  <div className="mt-1">
                    <div className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-gray-100 rounded-full border border-gray-200/50">
                      ...
                      <p className="text-[10px] text-gray-500 font-bold tracking-tight truncate max-w-30">{userData.email}</p>
                    </div>
                  </div>
                </div>
              </div>
                ...
                {[
                  ...
                  >
                    ...
                    <div className="relative z-10">
                      <p className="text-[8px] uppercase font-bold text-gray-500 mb-1 tracking-widest italic">{stat.label}</p>
                      <div className="flex items-center gap-1.5">
                        <span className={cn("font-bold uppercase italic drop-shadow-sm leading-none", stat.valueClass)}>{stat.value}</span>
              ...
              <div key={item.key}>
                ...
                <Link
                  ...
                  className={cn(
                    ...
                    isLogout ? "hover:bg-rose-50 text-rose-600" : isActive ? "bg-slate-50 text-orange-600 shadow-sm" : "hover:bg-slate-50 text-gray-700",
                  )}
                >
                  <div
                    className={cn(
                  ...
                  <span className={cn("flex-1 text-[13px] font-bold tracking-tight italic transition-colors", isActive ? "text-orange-600" : "")}>
                    {item.label}
                  </span>
                  {!isLogout && (
                    <ChevronRight
      ...
      <div className="px-6 py-3 bg-gray-50/50 rounded-b-4xl border-t border-gray-50 flex justify-center">
        <p className="text-[8px] font-bold text-gray-600 uppercase tracking-widest">Verified Protocol • {dayjs().format("YYYY")}</p>
      </div>
    </AppPopover>
  );
};
