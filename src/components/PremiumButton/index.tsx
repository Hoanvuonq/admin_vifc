...
const LoadingSpinner: FC = () => <span className="w-4 h-4 border-2 border-t-2 border-white border-t-transparent rounded-full animate-spin" />;
...
export const PremiumButton = ({
  ...
  return (
    ...
    >
      {variant !== "isLogin" && (
        <div className={cn("absolute inset-0 translate-y-full group-hover/btn:translate-y-0 transition-transform duration-500 z-0", style.bg)} />
      )}
      ...
      <div className="relative z-10 flex items-center gap-2">
        ...
        {Icon && (
          ...
          >
            {isLoading ? <LoadingSpinner /> : <Icon size={iconSize} strokeWidth={2.5} />}
          </div>
        )}
        {label}
        {children}
