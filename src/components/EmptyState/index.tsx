...
export const EmptyState = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative flex flex-col items-center justify-center bg-white/40 backdrop-blur-2xl rounded-[32px] md:rounded-2xl border border-dashed border-orange-200 shadow-[0_8px_32px_rgba(249,115,22,0.05)] my-4 itim-regular overflow-hidden",
        isFlashSale
          ? compact
            ? "py-8 md:py-10 px-4"
            : "pt-20 md:pt-32 pb-8 md:pb-10 px-4 md:px-12"
          : compact
            ? "py-8 md:py-10 px-4"
            : isTable
              ? "py-10 md:py-14 px-6 md:px-12"
              : "py-6 md:py-16 px-6 md:px-12",
        className,
      )}
    >
      ...
      >
        <div
          className={cn(
            ...
            compact ? "w-28 h-28" : isTable ? "w-48 h-48 md:w-56 md:h-56" : "w-40 h-40 md:w-48 md:h-48",
          )}
        >
      ...
      {showButton && (finalAction || link) && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className={cn(compact ? "mt-4" : "mt-8")}>
          {link ? (
            <Link href={link}>
              <PremiumButton
                variant="orange"
                ...
                className={cn(
                  ...
                  isFlashSale || isTable
                    ? "h-12 px-10 bg-linear-to-r from-orange-500 to-orange-600 hover:scale-105 shadow-orange-500/40 text-white"
