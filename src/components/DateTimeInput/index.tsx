    ...
    <div className={cn("space-y-2 w-full relative", className)} ref={containerRef}>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className={cn("text-sm font-bold italic tabular-nums", !value && "font-normal text-gray-500")}>{renderDisplayValue()}</span>
        </div>
      </div>
      ...
      {isOpen &&
        ...
        createPortal(
          <div
            className={cn(
              "fixed z-99999 datetime-portal-content bg-white border border-gray-100 shadow-2xl rounded-2xl p-6 flex flex-col overflow-hidden",
              ...
              {(!isTime || (isDate && isTime) || isDate) && (
                <div className="w-72 shrink-0">
                  <div className="flex items-center justify-between mb-6 px-1">
                    ...
                    <span className="text-sm font-bold text-gray-700 uppercase tracking-tight italic">{format(currentMonth, "MMMM yyyy", { locale: vi })}</span>
                    <button
                      type="button"
                  ...
                  <div className="grid grid-cols-7 gap-1 text-center mb-6 px-1">
                    {["T2", "T3", "T4", "T5", "T6", "T7", "CN"].map((d) => (
                      <span key={d} className="text-[10px] font-bold text-gray-500 py-1 uppercase">
                        {d}
                      </span>
                    ))}
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            isSameDay(day, selectedDate) ? "bg-gray-900 text-white shadow-lg scale-110 z-10" : "hover:bg-orange-50 text-gray-600",
                            isToday(day) &&
                ...
                <div className={cn("flex gap-6 relative", (!isTime || (isDate && isTime)) && "border-l border-gray-100 ml-10 pl-10")}>
                  {["Giờ", "Phút"].map((label, idx) => (
                    <div key={label} className="flex flex-col items-center">
                      ...
                      <div className={cn("relative h-80 w-16 bg-gray-50 rounded-2xl overflow-hidden border border-gray-100", isTime && "wh-60")}>
