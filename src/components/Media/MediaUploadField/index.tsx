...
import { ArrowUpRight, CheckCircle2, Eye, Globe, Layout, Lock, Plus, Trash2, Video as VideoIcon, File } from "lucide-react";
...
> = ({
  ...
  const SIZE_MAP = isVideo
    ? { sm: "w-52 h-32 rounded-2xl", md: "w-full h-48 aspect-video rounded-2xl", lg: "w-full aspect-video rounded-2xl" }
    : isBanner
      ? { sm: "w-64 h-24 rounded-xl", md: "w-full h-40 rounded-2xl", lg: "w-full h-60 rounded-2xl" }
      : { sm: "w-28 h-28 rounded-2xl", md: "w-40 h-40 rounded-2xl", lg: "w-56 h-56 rounded-2xl" };
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of Array.from(files)) {
      ...
      const newFile: CustomFile = {
        uid,
        name: file.name,
        status: onUploadApi ? "uploading" : "done",
        originFileObj: file,
        url: objectUrl,
        type: file.type,
  ...
  return (
    <div className={cn("flex items-center justify-start flex-wrap gap-5", className)}>
        ...
        {value
          ...
          .map((file, index) => (
            <motion.div
              ...
              className={cn(
                "flex flex-col gap-2",
                isBanner ? "w-full" : isPost ? (size === "md" ? "w-40" : size === "lg" ? "w-full" : "w-28") : size === "lg" ? "w-full" : "w-fit",
              )}
            >
              <div
                className={cn(
                  "relative group overflow-hidden border border-slate-100 bg-white shadow-sm hover:shadow-orange-500/10 hover:shadow-2xl",
              ...
              >
                <div className="relative w-full h-full">
                  {file.url && (
                    <div className="w-full h-full overflow-hidden">
                      {file.type?.includes("video") || file.url?.toLowerCase().endsWith(".mp4") || file.url?.toLowerCase().endsWith(".mov") ? (
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-center bg-slate-50 text-rose-500">
                          <File size={32} />
                          <span className="text-[10px] mt-2 font-bold uppercase text-slate-500">{file.name || "PDF Document"}</span>
                        </div>
                      ) : (
                  ...
                  <div className="absolute top-3 right-3 z-10">
                    <div className={cn("p-1.5 rounded-lg text-gray-700", GLASS_PANEL)}>{mode === "public" ? <Globe size={12} /> : <Lock size={12} />}</div>
                  </div>
                  {file.status === "done" && (
                ...
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 flex items-center justify-center gap-3 z-30 transition-all duration-300 translate-y-2 group-hover:translate-y-0">
                  ...
                  <button
                    type="button"
                    onClick={() => onChange(value.filter((f) => f.uid !== file.uid))}
                    className={cn(ACTION_BTN, "bg-orange-500 text-white hover:bg-orange-600")}
                  >
                    <Trash2 size={18} />
                  </button>
                </div>
        ...
        {value.filter((f) => f.uid).length < maxCount && (
          ...
          >
            ...
            <div className="relative z-10 flex flex-col items-center p-2 text-center">
              <div
                className={cn(
                  "w-14 h-14 rounded-2xl bg-white border border-slate-100 flex items-center justify-center text-gray-600 group-hover:text-orange-600 group-hover:shadow-xl group-hover:shadow-orange-500/20 group-hover:-rotate-12",
                  COMMON_TRANSITION,
                )}
              >
      ...
      <MediaLightbox mediaList={value as any} currentIndex={previewIndex} onChangeIndex={setPreviewIndex} onClose={() => setPreviewIndex(null)} />
    </div>
  );
};
