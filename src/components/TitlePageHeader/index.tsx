...
export const TitlePageHeader = ({
  ...
  return (
    <div
      className={cn("flex items-center select-none animate-in fade-in slide-in-from-left-4 duration-700", isXs ? "gap-3" : isSm ? "gap-4" : "gap-5", className)}
    >
      {Icon && (
      ...
      <div className={isXs ? "space-y-0.5" : isSm ? "space-y-0.5" : "space-y-1.5"}>
        ...
        >
          <span className={cn("drop-shadow-sm", (isTitleHighlight && "text-orange-500") || (isWhite && "text-white"))}>{title}</span>
          {highlightTitle && (
            <span className={cn("drop-shadow-[0_2px_2px_rgba(234,88,12,0.15)]", !isTitleHighlight && "text-orange-500 ")}>{highlightTitle}</span>
          )}
        </h1>
