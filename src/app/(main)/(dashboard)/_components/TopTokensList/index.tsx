      ...
      footer={
        <>
          <div className="flex items-center gap-1.5 px-3 py-1 bg-orange-50 rounded-full border border-orange-100">
            ...
            <span className="text-[9px] font-bold text-orange-600 uppercase tracking-widest">High Yield Assets</span>
          </div>
          <span className="text-[9px] font-bold text-gray-600 uppercase tracking-widest">Updated: {timeStr || "--:--:--"}</span>
        </>
      }
    >
      <div className="grid grid-cols-1 gap-4">
        {activeTokens.map((item, idx) => {
          ...
          return (
            <div
              key={item.tokenId}
              className="group/item relative bg-white rounded-2xl border border-slate-100 p-4 transition-all duration-500 hover:border-orange-200 hover:shadow-[0_10px_30px_-10px_rgba(249,115,22,0.12)] hover:-translate-y-1 flex items-center gap-5 overflow-hidden"
            >
              {/* Medal rank */}
              <div
                className={cn(
              ...
              <div className="flex-1 min-w-0 space-y-2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-col items-end">
                    ...
                    <span className="text-base font-bold text-orange-600 italic tabular-nums leading-none mt-0.5">{formatPrice(item.price)}</span>
                  </div>
                </div>
              </div>
