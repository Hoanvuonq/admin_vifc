          ...
          <span className="text-[10px] font-bold text-gray-500 uppercase italic">
            {activePools.length > 0 ? (
              <>
                ...
                {criticalCount > 0 && <span className="text-rose-600 ml-1 font-bold">• {criticalCount} CRITICAL</span>}
              </>
            ) : (
      ...
      <div className="grid grid-cols-1 gap-4">
        {activePools.map((item) => {
          ...
          return (
            <div
              key={item.poolId}
              className="group/pool relative bg-white rounded-2xl border border-slate-100 p-4 transition-all duration-500 hover:border-orange-200 hover:shadow-[0_10px_30px_-10px_rgba(249,115,22,0.08)] hover:-translate-y-0.5 flex flex-col gap-3 overflow-hidden"
            >
