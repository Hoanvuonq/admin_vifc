...
export const CardBox = ({
  ...
  return (
    <div
      className={cn(
        "bg-white rounded-2xl border border-slate-100 shadow-custom flex flex-col h-[650px] overflow-hidden group/list relative transition-all duration-500",
        height,
        className,
      )}
    >
      ...
      <div className="flex-1 relative overflow-hidden">
        <AnimatePresence mode="wait">
          {showLoading ? (
            ...
            >
              <SectionLoading isOverlay message="Loading..." size="sm" className="h-full" />
            </motion.div>
          ) : null}
        </AnimatePresence>
        ...
        <motion.div
          ...
          transition={{ duration: 0.5 }}
          className={cn("h-full overflow-y-auto p-6 custom-scrollbar bg-linear-to-b from-white to-orange-50/10", contentClassName)}
        >
          {children}
        </motion.div>
      </div>

      {footer && <div className="p-4 bg-white/80 backdrop-blur-md border-t border-orange-50 flex items-center justify-center gap-3 z-20">{footer}</div>}
    </div>
  );
};
