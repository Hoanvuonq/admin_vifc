...
export const ManagerUsersScreen = () => {
  ...
  const { users: apiUsers, isLoading, pagination } = useUsers(currentPage + 1, pageSize, selectedStatus, debouncedSearch);
    ...
    if (selectedUserToEdit) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          showToast(errData?.error?.message || "Failed to create user", "warning");
        }
      } catch (error) {
    ...
    <div className="min-h-screen space-y-6 animate-in fade-in duration-700 relative">
      ...
      >
        <PremiumButton
          ...
          className="h-11 px-5 rounded-2xl font-bold text-[11.5px] uppercase tracking-wider shadow-lg shadow-orange-500/20"
        />
      </AdminPageHeader>
      ...
      <CreateUserModal open={isCreateUserModalOpen} onClose={() => setIsCreateUserModalOpen(false)} onSave={handleSaveUser} />
