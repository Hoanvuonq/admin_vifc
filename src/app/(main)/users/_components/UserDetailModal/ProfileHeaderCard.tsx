...
export const ProfileHeaderCard: React.FC<ProfileHeaderCardProps> = ({
  ...
  return (
    <div className="relative p-4 rounded-2xl bg-linear-to-br from-gray-900 via-gray-800 to-orange-950 overflow-hidden shadow-2xl">
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
        <div className="relative group shrink-0">
          ...
          <div className="w-36 h-36 relative z-10 overflow-hidden rounded-2xl border-4 border-white/10 shadow-2xl transition-transform duration-500 group-hover:scale-[1.02]">
            <Image
              src={avatarUrl}
        ...
        <div className="flex-1 text-center md:text-left space-y-4">
          <div className="flex flex-wrap justify-center md:justify-start gap-2">
            ...
            {packages.map((pkg, i) => (
              <span
                key={i}
                className={cn("px-4 py-1 rounded-full text-[9px] font-bold uppercase shadow-lg flex items-center gap-1", getPackageBadgeStyle(pkg))}
              >
                GÓI MUA: {pkg}
              </span>
            ))}
