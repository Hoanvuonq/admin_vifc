...
export const getPaymentHistoryColumns = (): Column<PaymentRecord>[] => [
    ...
    render: (record: PaymentRecord) => (
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl overflow-hidden border border-gray-100 bg-gray-50 shrink-0">
          <ItemImage path={record.packageImage} productName={record.packageName} className="w-full h-full object-cover" />
        </div>
  ...
  },
  {
    ...
    render: (record: PaymentRecord) => <span className="font-mono text-sm font-bold text-slate-700">${record.price.toFixed(2)}</span>,
  },
  {
    header: "Date & Time",
  ...
  },
  {
    ...
    render: (record: PaymentRecord) => {
      ...
      return (
        <span className={`inline-flex items-center justify-center px-2 py-1 rounded-lg text-[10px] font-bold uppercase border ${badgeClass}`}>{label}</span>
      );
    },
  },
];
