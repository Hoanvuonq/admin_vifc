...
import { FormInput, ItemImage, PortalModal, PremiumButton, SectionHeader, SelectComponent, StatusBadge } from "@/components";
...
export interface RegistrationDetailModalProps {
  ...
  onReview: (id: string, payload: ReviewBookingPayload) => Promise<void>;
  isLoading?: boolean;
}
...
export const RegistrationDetailModal: React.FC<RegistrationDetailModalProps> = ({ isOpen, onClose, registration, onReview, isLoading = false }) => {
          ...
          <div className="flex items-center gap-2 shrink-0">
            {isPending && (
              <>
                ...
                />
                <PremiumButton label="Từ chối" size="sm" icon={XCircle} variant="rose" disabled={isSubmitting || isLoading} onClick={handleQuickReject} />
              </>
            )}
          </div>

          <div className="flex items-center gap-2.5 ml-auto shrink-0">
            <PremiumButton label="Đóng" size="sm" variant="gray" onClick={onClose} isLoading={isSubmitting || isLoading} />
            <PremiumButton label="Lưu phê duyệt" size="sm" icon={Save} variant="orange" onClick={handleSave} isLoading={isSubmitting || isLoading} />
          </div>
        </div>
      }
    >
      <div className="space-y-5 animate-in fade-in duration-300">
        <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/70 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3.5 min-w-0">
            <ItemImage
              path={
                registration.users?.avatar_url ||
                registration.avatar_url ||
                `https://api.dicebear.com/7.x/identicon/svg?seed=${encodeURIComponent(registration.email || registration.full_name || "guest")}`
              }
            ...
            <div className="space-y-1 min-w-0">
              <div className="flex items-center gap-2">
                <h3 className="text-base font-extrabold text-gray-900 truncate">{registration.full_name || "Học viên chưa đặt tên"}</h3>
                <span className="text-[10px] bg-orange-100 text-orange-700 font-mono font-bold px-2 py-0.5 rounded-full shrink-0">{orderCode}</span>
              </div>
              <p className="text-xs text-gray-600 flex items-center gap-1.5 select-all truncate">
                <Mail size={12} className=" text-gray-700 shrink-0" /> {registration.email}
              </p>
              {registration.phone && (
                <p className="text-xs text-gray-600 flex items-center gap-1.5 select-all truncate">
                  <Phone size={12} className=" text-gray-700 shrink-0" /> {registration.phone}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col sm:items-end gap-1.5 text-xs shrink-0">
            <span className=" text-gray-700 text-[11px] font-bold uppercase">Trạng thái hiện tại:</span>
        ...
        <div className="space-y-3">
          ...
          <div className="p-4 rounded-2xl bg-white border border-gray-200/80 shadow-xs space-y-3 text-xs">
            <div className="flex items-start justify-between gap-2">
              <div>
                <span className=" text-gray-700 block text-[10.5px] font-bold uppercase">Tên khóa học / Dịch vụ</span>
                <span className="font-extrabold text-gray-900 text-sm mt-0.5 block">{registration.booking_title}</span>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-2.5 border-t border-gray-100 text-xs">
              <div>
                <span className=" text-gray-700 block text-[10.5px] font-bold uppercase">Mã đơn booking</span>
                <span className="font-mono font-bold text-gray-800 mt-0.5 block">{orderCode}</span>
              </div>

              <div>
                <span className=" text-gray-700 block text-[10.5px] font-bold uppercase">Công ty / Đơn vị</span>
                <span className="font-semibold text-gray-700 mt-0.5 block">{registration.company || "Cá nhân"}</span>
              </div>

              <div>
                <span className=" text-gray-700 block text-[10.5px] font-bold uppercase">Thời gian gửi</span>
        ...
        {registration.note && (
          <div className="space-y-2">
            <SectionHeader icon={MessageSquare} title="Ghi chú từ Khách hàng" description="Yêu cầu riêng hoặc lưu ý từ người đăng ký" size="sm" />
        ...
        <div className="space-y-3.5 pt-2 border-t border-gray-100">
          ...
          <div className="space-y-3.5">
            <div className="space-y-2">
              <label className="text-[12px] font-bold text-gray-700 ml-1 flex items-center gap-1">Chọn trạng thái phê duyệt</label>
              <SelectComponent
