export const convertBlockNoteHTMLToTailwindAndClean = (html: string): string => {
...
export const convertTailwindAndCleanToBlockNoteHTML = (html: string): string => {
    ...
    let processed = html.replace(/(<(?:p|h1|h2|h3|h4|h5|h6|li|ol|ul|div|strong|b|em|i)[^>]*)/g, (match) => {
        ...
        if (dataAttrs) {
            if (updatedTag.endsWith("/")) {
                updatedTag = updatedTag.substring(0, updatedTag.length - 1) + dataAttrs + " /";
            } else {
                updatedTag = updatedTag + dataAttrs;
            }
        }
