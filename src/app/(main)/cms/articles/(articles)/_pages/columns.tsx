...
export const getColumns = (
    ...
    render: (news: NewsItem) => (
      <div className="flex items-start gap-3 py-1 max-w-full">
        <div className="relative select-none shrink-0 w-28 h-20 rounded-xl overflow-hidden border border-gray-100 bg-gray-50 shadow-sm">
          <ThumbnailImage src={news.thumbnail || "https://api.dicebear.com/7.x/shapes/svg?seed=onchainpass"} alt={news.title} />
        </div>
        <div className="flex flex-col min-w-0 flex-1 space-y-1">
          ...
          <span className="text-[11px]  text-gray-700 font-medium line-clamp-2 leading-relaxed" title={news.description}>
            {news.description}
          </span>
        </div>
      </div>
...

  {
    ...
    render: (news: NewsItem) => <StatusBadge status={news.status || "PUBLISHED"} variant="premium" />,
  },
  {
    header: "Created Date",
    ...
    render: (news: NewsItem) => (
      <div className="flex items-center gap-1.5 text-gray-500">
        <Clock size={12} className=" text-gray-700 shrink-0" />
  ...
  },
  {
    ...
    render: (news: NewsItem) => (
      <div className="flex items-center justify-center gap-1.5" onClick={(e) => e.stopPropagation()}>
        <ActionTooltipBtn onClick={() => handleEditNews(news)} icon={<Edit size={13} />} color="orange" tooltip="Edit article Old" />
        <ActionTooltipBtn onClick={() => handleEditNewsBlockNote(news)} icon={<Edit size={13} />} color="orange" tooltip="Edit article Block Note" />
        <ActionTooltipBtn onClick={() => handleDeleteNews(news.id)} icon={<Trash2 size={13} />} color="red" tooltip="Delete article" />
      </div>
    ),
  },
];
