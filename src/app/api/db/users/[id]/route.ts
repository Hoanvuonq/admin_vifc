...
export async function PATCH(
  ...
  try {
    ...
    const { subscription_plan_id, role_id, role_name, full_name, status, avatar_url } = body;
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // 2. Update Subscription Plan if provided
      if (subscription_plan_id) {
        ...
        if (activeSub) {
          // Update existing active subscription with new plan
    ...
    // Re-fetch the updated user to return the new state
    const user = await prisma.user.findUnique({
      where: { id },
      select: {
        id: true,
